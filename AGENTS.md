...
```
问题（P05 电子围栏）    长期存在，来自现象，用编号追溯，跨分支
功能（松杆主动刹车）    短期，来自方案，用分支承载，做完即删
```

**分支承载功能，编号追溯问题。** 两个轴互不打架，这是整套约定的基础。

一个「功能」的判据很实用：**能独立验证、独立合并的一次改动**。

```
✅ 是功能：掉桨检测 / 偏航跟踪 MOT_FAIL_YTRK / 松杆刹车 / 惰走仿真
❌ 不是：  「P04」「围栏问题」——那是问题，一个问题往往含 3–5 个功能
```

若一项改动**不能单独跑回归、单独通过判据**，它还不是一个功能：要么再拆，要么和别的并成一个。

### 唯一的硬规矩：寿命

> **分支随功能生，随功能合，合完即删。超过 3 天没合，要么拆小，要么并回主干重来。**

这条一破就会退化。`dev-p04` 是活标本——它本身不是坏主意，**是一条活得太久的功能分支**：久到装进了 P06 的三点定圆、两条电池失效保护提交和三项共享工具改动，14 个独有提交里只有 7 个是它本来要做的事。

### 为什么不按问题或按领域分

两版都试过，都垮了：

**按问题分**——见上面 `dev-p04` 的实例。问题的生命周期是**月**，分支跟着它走必然活太久。

**按领域分**（`dev-control` / `dev-planning`）——P05 电子围栏要同时改 `AC_Avoid`/`AC_PosControl`（限速器）与 `AC_Fence`（几何、越界动作），拆到两条分支上不仅要来回切，**更没法验证**：围栏修复必须限速器与几何同时在场才能测，拆开后哪条分支都跑不出正确结果。P07 圆周抽动同理，跨 `AC_ArcNav` 与姿态环。**一个连自己都验证不了的划分没有存在价值。**

按功能分则没有这个问题：一个功能的全部文件天然在同一条分支上。
...
```
pr-shed-detect        上游风格（子模块里可见 pr-canfd-lencheck、pr-cxx-wrappers）
feat/shed-detect      也可以，只要全队统一
dev-p04               ✗ 问题编号进分支名，会诱使分支跟着问题的生命周期走
```

### 共享层与参数门控

- **共享层的改动优先直接落 `dev-algo`**：`Tools/`、`libraries/SITL/`、`libraries/AP_HAL_SITL/`、`docs/`、`AGENTS.md`。统计近 80 个提交，真正被多个问题编号动过的文件只有 `Tools/eft_issue_repro/reproduce.py` 与 `regression.py`——**分支之间的耦合几乎全部来自共享层**，把它压在功能分支上既让别人用不上，又保证将来冲突。
- **新能力一律参数门控、默认关闭**：`MOT_FAIL_RPM`、`MOT_FAIL_ROVR`、`MOT_FAIL_YTRK`、`MOT_STOP_DECL`、`SIM_ENGINE_TAU`、`MOT_FAIL_IDX` 的默认值都使行为与合并前逐位相同。这样功能分支可以早合，不必等真机验证完才敢进主干。

### 并发靠目录，不靠分支

见第 9 节。分支解决不了两个 agent 共用一个目录的问题。
