...
**一个问题的解决不跨分支。** 现场按问题一个一个推进，一个问题的全部改动就在 `dev-algo` 上依次提交完成。
...
这是本节最容易搞反的一条。「这架飞机当时飞的是哪个状态」由 **tag** 回答，而且日志里能直接对上：

```
v3.0.33  ← 日志 00000219（2026-08-20）
v3.0.34  ← 日志 00000211（2026-09-02）
```

tag 不可变、零成本、与日志中的 `FIRMWARE_VERSION` 一一对应。**分支不承担这个职责**——分支会动，而已经飞过的状态不能动。
...
```
改既有参数的默认值 / 重构共享路径 / 动 EKF 这类关不掉的东西
```

命名 `pr-<一句话>`，跟上游习惯（子模块里可见 `pr-canfd-lencheck`、`pr-cxx-wrappers`）。**短命，做完即合即删。**

其余一切都不需要分支，因为本仓库的新能力一律**参数门控、默认关闭**：
...
默认值使代码进主干后**行为与合并前逐位相同**，启用与否是参数的事。上游 ArduPilot 也是这么做的——EKF、控制、导航、驱动全在一个 `master` 上，没有任何按子系统或按问题划分的长期分支。
...
| 方案 | 为什么垮 |
| :-- | :-- |
| **按问题分**（`dev-p04`） | 问题的生命周期是**月**，分支跟着走必然活太久。`dev-p04` 活到装进了 P06 的三点定圆、两条电池提交和三项共享工具改动，14 个独有提交里只有 7 个是它本来要做的事 |
| **按领域分**（`dev-control`/`dev-planning`） | P05 电子围栏要同时改 `AC_Avoid`/`AC_PosControl`（限速器）与 `AC_Fence`（几何），拆到两条分支上不仅要来回切，**更没法验证**——围栏修复必须两者同时在场才能测。P07 跨 `AC_ArcNav` 与姿态环，同理 |
| **按功能分** | 不违反验证性，但与现场"按问题一个个解决"的实际节奏不符：一个问题拆出 5–8 个功能，就要开合 5–8 次分支，而这些分支之间没有并行需求 |

共同的教训：**隔离是有成本的，而本仓库并不需要那么多隔离**。统计近 80 个提交，真正被多个问题编号动过的文件只有 `Tools/eft_issue_repro/reproduce.py` 与 `regression.py`——算法代码基本单问题，不同问题本来就不太会碰同一个文件。
