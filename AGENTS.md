...
### 7.6b 目标导向：一条分支一个目标，可多人参与

**分支的单位是「目标」——一个可验证的成果**，不是一个人的工作量。目标需要两个专长时，两个人在同一条分支上做，各做各的一层，**分支上合成的才是可验证的状态**。

```
wip-P05-fence-brake   目标：松杆后能在围栏前刹住
                      参与：控制（限速器 AC_PosControl）+ 规划（围栏几何 AC_Fence）

wip-P04-land-timeout  目标：告警后无人响应也能自动落地
                      参与：控制（状态机）+ 规划（可行域传递）
```

这两个目标都需要两个专长同时在场才验证得了——这也正是按领域切分支会垮掉的原因（见 7.1）。
...
```bash
git switch -c wip-P05-fence-brake dev-algo
git commit --allow-empty -F - <<'EOF'
P05: 开分支——松杆后能在围栏前刹住

目标  STABILIZE / ALT_HOLD / POSHOLD / DRIFT 下松杆，飞机能在围栏前主动刹停
判据  regression.py 的姿态层围栏六条模式条目全绿
参与  控制（限速器 AC_PosControl）+ 规划（围栏几何 AC_Fence）
起点  dev-algo <sha>
EOF
git push -u origin wip-P05-fence-brake
```

用空提交而不是单独的目标文件：**不会冲突**（每条分支都改同一个目录必然冲突），且**合并后目标声明留在 `dev-algo` 历史里**——半年后问"当初为什么做这个"，`git log --grep="^P05"` 第一条就是目标与判据。

> `git branch --edit-description` 也能存描述，但写在本地 `.git/config`，**不随推送走**，多机协作用不了。

查所有人在做什么：

```bash
for b in $(git branch -r --list 'origin/wip-*'); do
  printf "%-34s %s\n" "${b#origin/}" "$(git log --format='%s' "$b" | tail -1)"
done
```

#### 目标必须自带验收判据

这是目标导向法真正的门槛：**没有判据，两个人对"做完了"的理解不同，分支就不知道什么时候该死。**

本仓库有现成机制：**一个目标 = 一条（或一组）能从红转绿的回归条目**。`regression.py` 里那 21 条，每一条都是一个已达成目标的判据。新目标开分支时**先把判据写进去（可以先是失败的）**，分支的终点就明确了——那条判据转绿。

#### 多人共用一条分支的四条

1. **各自独立目录**——见 7.7，这条不变且更要紧；
2. **提交前先同步**：`git fetch origin && git merge --no-edit origin/wip-<...>`；
3. **小提交、勤推**——攒着不推，另一个人就是在旧基础上干活；
4. **判据先写**——见上。
