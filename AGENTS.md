...
问题编号（P01…P08）与代码边界不是一回事，两次都验证过：

- **按问题分支撑不住**。`dev-p04` 名义上只做单动力失效，实际装进了 P06 的三点定圆、两条电池失效保护提交，以及三项共享工具改动——14 个独有提交里只有 7 个是 P04 本身。
- **按领域分也撑不住**。围栏（P05）看着属于路径规划，实质是限速器设计，是控制问题；而 P01/P02/P05/P06/P07 在代码上共用 `AC_PosControl`、`AC_AttitudeControl`、`mode_auto.cpp`，按领域切只会让它们在同一批文件上撞车。

因此按**代码归属**划分：

| 分支 | 拥有的代码 | 覆盖的问题 |
| :-- | :-- | :-- |
| `dev-algo` | 主干，以及下面列出的**全部共享层** | — |
| `dev-motors` | `libraries/AP_Motors/`、`ROMFS_custom/scripts/motor_*` | P04 |
| `dev-estim` | `AP_Compass`、`AP_NavEKF3`、`AP_AHRS` | P03 |
| `dev-guidance` | `AC_WPNav/`、`AC_Fence/`、`AC_PosControl`、`AC_AttitudeControl`、`ArduCopter/mode_*` | P01、P02、P05、P06、P07 |

P01/P02/P05/P06/P07 合在一条分支上是有意的：它们本来就在同一批文件上工作，分开只增加合并成本而不带来隔离收益。
...
把共享层的改动压在专题分支上，等于让别的问题用不上它，同时保证将来合并时冲突。反过来，共享层先落 `dev-algo`、各分支再同步过去，分支之间就几乎不会碰同一个文件。

### 跨领域的问题怎么处理

确实有问题跨多个分支（例如 P08 动力系统不一致性，既涉及 `AP_Motors` 又影响姿态配平）。规则是：

1. **共用的部分先落 `dev-algo`**，各分支从那里同步；
2. **专题分支之间永不直接互相合并**——那会把两条线的历史缠在一起，之后谁也理不清；
3. 若一处改动无法拆分，就把它整个放在 `dev-algo` 上做，不进专题分支。

### 节奏

- 专题分支**每天从 `dev-algo` 同步一次**，单向；问题收口即合回并删除分支。
- 新能力一律**参数门控、默认关闭**——`MOT_FAIL_RPM`、`MOT_FAIL_ROVR`、`MOT_FAIL_YTRK`、`MOT_STOP_DECL`、`SIM_ENGINE_TAU` 默认值都使行为与合并前逐位相同。这是分支能安全早合的前提。
