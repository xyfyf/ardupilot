...
本文档由以下两个原始文档于 2026-06-08 合并去重而来（原始文件已删除，可通过 git 历史回溯）：

| 原文件 | 内容侧重 |
| :--- | :--- |
| `LTE 4G 使用说明.md` (636 行) | 物理链路、PPP、踩坑、公网中转、外场 SOP、商业化 |
| `LTE_UOM_SOP.md` (278 行) | UOM 五个 ID、MQTT 主题/JSON 格式、激活流程 |

如需查阅原始版本，请使用：

```bash
git log --diff-filter=D --name-only -- "libraries/AP_Scripting/eft/*.md"
```
