...
local function handle_gcs_control_message(msgid, decoded_msg)
    ...
    if msgid == 505 then  -- WEIGHT_CALIBRATION
        ...
        gcs:send_text(6, string.format("GCS: LED控制更新 R:%d L:%d", gcs_control.led_brightness_right, gcs_control.led_brightness_left))
    ...
    elseif msgid == 507 then  -- PUMP_CALIBRATION_CMD
        ...
        gcs:send_text(6, string.format("GCS: 校准命令 0x%02X", gcs_control.pump_calibration_cmd))
    ...
    elseif msgid == 509 then  -- SPRAY_SYSTEM_PARAMS
        ...
        gcs:send_text(6, string.format("GCS: 喷洒参数 速率:%d 宽度:%d", gcs_control.spray_rate_from_gcs, gcs_control.spray_width_from_gcs))
    ...
    elseif msgid == 511 then  -- SPREADER_CONTROL
        ...
        gcs:send_text(6, string.format("GCS: 播撒器控制 电机:%d 阀门:%d", gcs_control.spreader_motor_pwm, gcs_control.spreader_valve_pwm))
    end
end
...
-- 启动脚本
gcs:send_text(6, "SYS: PMU控制器v8.1已启动")
return update, 1000
