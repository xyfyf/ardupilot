...
if not can_driver then
    gcs:send_text(0, "TR24: 错误 - 未找到 Scripting CAN 接口 (请设置 CAN_D1_PROTOCOL=10)")
    return
end
...
gcs:send_text(6, "TR24: CAN 驱动初始化成功, 监听 ID=0x0301")
...
local function try_init_prx()
    ...
    for j = 0, sensor_count - 1 do
        ...
        if device and (device:type() == PARAM_NUM_LUA_PRX_BACKEND) then
            ...
            gcs:send_text(6, "TR24: Proximity 后端初始化成功")
            return true
        end
    end
    return false
...
local function update()
    ...
    if not try_init_prx() then
        ...
        if not prx_wait_logged then
            gcs:send_text(3, "TR24: 等待 Proximity 后端... (请确认 PRX1_TYPE=15)")
    ...
    if avoidance_enabled ~= last_avoidance_state then
        if avoidance_enabled then
            gcs:send_text(6, string.format("TR24: 雷达避障已开启 (CH%d PWM=%d)", RC_CHANNEL, pwm or 0))
        elseif last_avoidance_state ~= nil then
            gcs:send_text(6, string.format("TR24: 雷达避障已关闭 (CH%d PWM=%d)", RC_CHANNEL, pwm or 0))
        end
    ...
    if (now_ms - last_log_ms) >= LOG_INTERVAL_MS then
        last_log_ms = now_ms
        local state_str = avoidance_enabled and "开启" or "关闭"
        if last_dist_m >= 0 then
            gcs:send_text(6, string.format("TR24: 距离=%.2fm | 避障=%s", last_dist_m, state_str))
        else
            gcs:send_text(6, string.format("TR24: 距离=无数据 | 避障=%s", state_str))
        end
    end
end
...
local function protected_wrapper()
    ...
    if not ok then
        gcs:send_text(0, "TR24: 运行错误: " .. tostring(err))
