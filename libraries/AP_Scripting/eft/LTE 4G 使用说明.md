# Air780E + ArduPilot LTE 4G 蜂窝数传与远距离遥控全解指南

## UOM 云平台 · 五个 ID 说明（2.3 变量说明）
...
| **fcu_id** | 飞控设备在平台上的唯一标识符 | `DRONE-001` | 激活 JSON `{"fcu_id":"…","timestamp":…}`；主题后缀 `{fcu_id}`；MQTT Client ID |
| **uas_id** | 航空器实名登记号（机体 ID，Remote ID） | 常与 fcu_id 相同 | 遥测 JSON 字段 `uas_id`；MP 写入 `LTE_UAS_W01~10` |
| **operator_id** | 运营人/操控员登记号（CAA 类） | 平台分配登记号 | 遥测 JSON 字段 `operator_id`；MP 写入 `LTE_OP_W01~10` |
...
本文档由一线飞控研发工程师总结，详细记录了在 STM32H7 平台（以 EFT-CAAC 为例）上集成合宙 Air780E 4G 模组（AT固件）的全流程。本指南不仅涵盖了常规的 `LTE_modem.lua` 脚本部署与 PPP 网络栈配置，还深度剖析并解决了 STM32H7 芯片底层 DMA 锁死勘误与 ADC 硬件冲突导致的数传丢包难题。

**适用范围**：ArduPilot 4.7+ / ChibiOS / STM32H7 系列飞控（尤其是采用自定义 hwdef 的国产飞控板）。
...
### 1.2 接线方法（以 `SERIAL1` 对应物理口为例）

**⚠️ 极度隐蔽的硬件级大坑（换线原因）**：
原本 4G 模块应该接在常规的数传接口（如 `COM1`），但在 EFT-CAAC 等采用 STM32H743 的部分飞控上，`COM1` 对应的物理引脚（PA2/PA3）与底层 ADC 引脚存在硬件复用冲突。如果在该口开启 DMA 进行高速 PPP 通信，会触发芯片级 DMA 锁死勘误（Errata 2.20.6），导致疯狂丢包、死机、以及无法联网（详见文末“坑 7”）。

**本文后续参数统一按 `SERIAL1_*` 编写**：请把 Air780E 接到**你这块飞控在 `hwdef.dat` 的 `SERIAL_ORDER` 里排在第 1 路的那组 UART**（丝印可能是 RSV1、Telem1 等，以原理图为准）。**EFT_CAAC 出厂映射**：`SERIAL1` = **RSV1（USART2，PA2/PA3）**；若你实际把模块接在 **GPS1（USART1，PA9/PA10）**，在出厂固件里那一路是 **`SERIAL3`**，请把全文中的 `SERIAL1` / `BRD_SER1` 全部改回 `SERIAL3` / `BRD_SER3`，否则参数与接线不一致。

`SERIAL1` 在参数表中就对应下文 `SERIAL1_PROTOCOL` / `SERIAL1_BAUD` 所控制的那路物理 UART。

```
飞控 SERIAL1 口     Air780E 开发板
─────────           ──────────────
   TX  ──────────>     RX
   RX  <──────────     TX
   GND ──────────>     GND
...
如果不添加 PPP 支持就直接使用 `LTE_PROTOCOL = 48`（PPP 模式），会出现以下死循环：
...
```
# Enable PPP networking backend for LTE modem support
...
> **注意**：如果使用 ArduPilot 在线自定义固件构建器 (https://custom.ardupilot.org)，
> 需要在构建选项中勾选 `Networking` 和 `PPP` 两个 Feature。

---

## 三、参数配置（分阶段，每阶段之间必须重启）

### 阶段一：开启 Lua 脚本引擎和虚拟串口

```
SCR_ENABLE      = 1         # 启用 Lua 脚本
SCR_HEAP_SIZE   = 200000    # Lua 内存池（LTE 脚本较大，建议 200KB+）
...
LTE_SERPORT     = 0         # ⚠️ 坑！这里填 0，不是 1（见下方说明）
LTE_SCRPORT     = 0         # ⚠️ 坑！这里填 0，不是 1（见下方说明）
...
NET_P1_IP1      = x         # 服务器公网 IP 第 2 段
NET_P1_IP2      = x         # 服务器公网 IP 第 3 段
NET_P1_IP3      = x         # 服务器公网 IP 第 4 段
...
不编译进去就使用 PPP 模式，表现为：反复 `connected` → 10秒 `timeout` → `reset` → 死循环。
...
2. **分配独占 DMA**：在 `hwdef.dat` 中，将其它不重要的外设和串口设为 `NODMA`（例如 COM1/2 设为 NODMA），并给 LTE 模块所在的干净串口分配独占 DMA 通道（例如在文件末尾增加 `DMA_NOSHARE SPI1* SPI2* USART1*`）。
...
PPP[0]: reconnecting                   ← 飞控底层 PPP 驱动开始 LCP/IPCP 协商
NET: IP      10.x.x.x                  ← 运营商分配了 IP 地址
NET: Mask    255.255.255.255           ← 子网掩码
NET: Gateway 10.x.x.x                 ← 默认网关
```

看到 `NET: IP` 就表示飞控已经成功通过 4G 网络联网。
...
# 网络出口（填写你的公网服务器信息）
NET_P1_TYPE      = 1         # 1=UDP client, 2=UDP server, 3=TCP client, 4=TCP server
NET_P1_PROTOCOL  = 2
...
### 8.5 飞控参数（以服务器 IP `8.145.61.235` 为例）
...
- [ ] 固件已编译 `AP_NETWORKING_BACKEND_PPP 1`
- [ ] `NET_ENABLE = 1`
- [ ] `SCR_SDEV1_PROTO = 48`
- [ ] `LTE_SERPORT = 0`，`LTE_SCRPORT = 0`（0-based 索引）
- [ ] 飞控日志出现 `NET: IP xxx.xxx.xxx.xxx`
...
### 10.1 Mission Planner 显示"连接质量低/丢包严重"怎么办？
...
1. **端口隔离（初级方案，目前适用）**
   ...
   - 客户A：出厂参数配置 `NET_P1_PORT = 15001`
   - 客户B：出厂参数配置 `NET_P1_PORT = 15002`
...
### 13.5 汇报建议话术（One-Pitch）
> *"老板，我们目前已经成功攻克了底层的硬件兼容难题，将 4G 蜂窝通信原生地集成到了我们的飞控架构中。这套方案不仅能让我们**把单机的数传成本削减 90% 以上**，更重要的是，它彻底打破了通信距离的瓶颈——**只要有手机信号的地方，我们就能远程控制飞机**。我们不仅能借此推出支持超视距飞行的行业无人机，还可以很方便地接入我们自己的云端指挥系统，这绝对是我们下一代产品的核心卖点。"*

---

## 十四、外场测试工程师 (Field Test Engineer) 连网与测试 SOP
...
### 14.2 Mission Planner 远程连接步骤（核心）
...
### 14.3 外场飞行注意事项与常见排障
- **安全保底（极其重要）**：在最初的几次外场 4G 测试中，**强烈建议测试工程师同时带上物理遥控器（如 ELRS 或 接收机）作为保底控制链路**。一旦飞到 4G 信号盲区导致连接断开，可以通过物理遥控器随时接管切回 Loiter 或触发 RTL (一键返航)。
