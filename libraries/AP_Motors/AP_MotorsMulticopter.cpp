...
const AP_Param::GroupInfo AP_MotorsMulticopter::var_info[] = {
    ...
    // @Description: How much of the yaw demand the degraded allocator chases once a motor has been removed. Throttle, roll and pitch are solved exactly first; only the freedom left over after that is spent on yaw, so unlike MOT_FAIL_YAW this cannot take authority away from attitude - that is the whole reason it is safe to try where MOT_FAIL_YAW was not, that one having put yaw back into the demand for the controller to chase and crashed the airframe in 2 m/s of wind. 0 keeps the existing behaviour, where the leftover freedom drives the parasitic yaw moment toward zero rather than toward the demand. 1 chases the demand in full. What it buys is a slower sweep, not a held heading: whatever this is set to, a hexacopter down one motor cannot hold heading in steady state - the four-constraint trim sits on the edge of the feasible set - and the measured heading error is unchanged at about 45 degrees either way. What changes is how fast the nose gets there. Measured in SITL on the worst-case motor with the detector in the loop: at 4 m/s of crosswind mean yaw rate falls 19.1 to 6.9 deg/s and peak drift 179 to 137 degrees, at no cost to attitude - steady roll actually improves 2.00 to 1.93 degrees and peak pitch error 3.19 to 2.69. At 8 m/s the mean falls further, 17.1 to 5.2 deg/s with drift 180 to 103 degrees, and here it is paid for: steady roll error rises 5.34 to 6.68 degrees, still inside the 10 degree criterion, with peak roll and horizontal drift unchanged and no motor saturation in any run. The pattern to expect is that the mean improves most where there is margin to spend and the roll cost appears where there is not. Requires MOT_FAIL_ALLOC=1 and a non-zero MOT_FAIL_YSUP, which supplies the weight this rides on. Unvalidated on a real airframe - default off.
    // @Range: 0 1
    // @User: Advanced
