...
const AP_Param::GroupInfo AP_MotorsMulticopter::var_info[] = {
    ...
    // @Description: How hard the degraded allocator works to keep the residual yaw moment small. With yaw out of the demand the remaining motors have freedom to spare - many thrust sets deliver the same throttle, roll and pitch - and plain minimum norm picks one without regard for how much yaw moment is left over. Raising this biases that pick toward less of it. A hexacopter cannot hold heading on five motors whatever this is set to; what this buys is a slower rotation, not a stopped one. Measured in SITL on the worst-case motor with 4 m/s of crosswind: yaw rate 22.1 deg/s at 0, 19.3 at 0.5, 18.0 at 0.7, 17.0 at 0.95, against a run-to-run scatter of 0.2 - so about a fifth of the rotation goes away, with diminishing returns above 0.7. Roll is not paid for it: peak roll error stays at 7.0 deg throughout, and steady roll error is lowest at 0.5 (0.15 deg against 0.21 at zero), rising again beyond it. 0.5 is the point where both are best. Note the static allocation analysis predicts a larger gain than this - it holds only at the exact hover trim, and the vehicle in flight is never there. Defaults to 0.5 on that evidence, extended across every failure position and a range of winds: yaw rate drops 8.5 to 12.7 percent whichever of the six motors goes, peak roll error is unchanged at all six, and steady roll error improves at five of them - the worst case across positions going from 0.34 to 0.23 deg. The gain grows with the wind rather than shrinking: at 8 m/s, steady roll error goes 1.00 to 0.77 deg and peak 4.21 to 3.89. The two places it reads worse (0.18 to 0.22 deg on one motor, 0.04 to 0.06 in still air) are both on values small enough not to matter, and neither touches the worst case. All sixteen runs stayed airborne. Set 0 to get the plain minimum-norm pick back. This suppresses the parasitic yaw moment, with a target of zero; it is not MOT_FAIL_YAW, which puts yaw back into the demand and asks the controller to track it.
    // @Range: 0 1
    // @Increment: 0.05
    AP_GROUPINFO("FAIL_YSUP", 52, AP_MotorsMulticopter, _fail_yaw_suppress, 0.5),
