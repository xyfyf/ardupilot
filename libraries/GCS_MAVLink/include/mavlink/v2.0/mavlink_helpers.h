...
/*
  ...
  - mav_tx_magic_override: when non-zero, replaces the MAVLink2 start-of-frame
    magic byte (normally MAVLINK_STX = 0xFD) on all channels.
  - mav_tx_magic_override_chan[]: per-channel default/override magic byte.
    Used when mav_tx_magic_override is zero.
  - mav_tx_crc_override_enable: when non-zero, forces the 2-byte CRC field of
    transmitted frames to mav_tx_crc_override_value instead of the computed checksum.
  - mav_tx_crc_override_value: the 16-bit CRC value used when the override is enabled.
  Definitions live here because this file is the single compiled unit for the
  MAVLink helpers (MAVLINK_SEPARATE_HELPERS).
*/
uint8_t mav_tx_magic_override = 0;
uint8_t mav_tx_magic_override_chan[MAVLINK_COMM_NUM_BUFFERS] = {};
uint8_t mav_tx_crc_override_enable = 0;
uint16_t mav_tx_crc_override_value = 0;
/* Debug counter: incremented every time the send path actually applies a
   magic/CRC override to an outgoing MAVLink2 frame. If this stays 0 after the
   override is enabled, the patched helper is NOT compiled into the firmware. */
volatile uint32_t mav_tx_override_hits = 0;

MAVLINK_HELPER uint8_t _mav_get_tx_magic(uint8_t chan)
{
	if (mav_tx_magic_override != 0) {
		return mav_tx_magic_override;
	}
	if (chan < MAVLINK_COMM_NUM_BUFFERS && mav_tx_magic_override_chan[chan] != 0) {
		return mav_tx_magic_override_chan[chan];
	}
	return MAVLINK_STX;
}

MAVLINK_HELPER void mavlink_set_channel_magic_override(uint8_t chan, uint8_t magic)
{
	if (chan < MAVLINK_COMM_NUM_BUFFERS) {
		mav_tx_magic_override_chan[chan] = magic;
	}
}
						    ...
						    uint8_t min_length, uint8_t length, uint8_t crc_extra)
{
	...
	// ArduPilot custom: apply runtime MAVLink2 framing override (immediate effect)
	if (!mavlink1) {
		const uint8_t tx_magic = _mav_get_tx_magic(chan);
		if (tx_magic != MAVLINK_STX) {
			buf[0] = tx_magic;
		}
		if (mav_tx_crc_override_enable != 0) {
			ck[0] = (uint8_t)(mav_tx_crc_override_value & 0xFF);
			ck[1] = (uint8_t)(mav_tx_crc_override_value >> 8);
		}
		if (tx_magic != MAVLINK_STX || mav_tx_crc_override_enable != 0) {
			mav_tx_override_hits++;
		}
	}
...
MAVLINK_HELPER uint16_t mavlink_msg_to_send_buffer(uint8_t *buf, const mavlink_message_t *msg)
{
	...
	// ArduPilot custom: apply global runtime MAVLink2 framing override only
	if (msg->magic != MAVLINK_STX_MAVLINK1) {
		if (mav_tx_magic_override != 0) {
			buf[0] = mav_tx_magic_override;
		}
		if (mav_tx_crc_override_enable != 0) {
			ck[0] = (uint8_t)(mav_tx_crc_override_value & 0xFF);
			ck[1] = (uint8_t)(mav_tx_crc_override_value >> 8);
		}
		if (mav_tx_magic_override != 0 || mav_tx_crc_override_enable != 0) {
			mav_tx_override_hits++;
		}
	}
...
{
	mavlink_status_t *status = mavlink_get_channel_status(chan);
	if (status->parse_state == MAVLINK_PARSE_STATE_UNINIT ||
	    status->parse_state == MAVLINK_PARSE_STATE_IDLE) {
		const uint8_t tx_magic = _mav_get_tx_magic(chan);
		if (tx_magic != MAVLINK_STX && c == tx_magic) {
			c = MAVLINK_STX;
		}
	}
