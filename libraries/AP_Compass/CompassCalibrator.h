...
#define COMPASS_CAL_NUM_SAMPLES             100     // 实际采集并用于拟合的样本数
// 间距公式用虚拟 N 计算，N 越小，要求点与点之间的距离越大（转的角度越多）。
// N=100 对应的 min_distance，使得在正常地磁强度下，收集 50 个点正好需要旋转约 360°。
// 这保证了水平方向能覆盖完整一圈，垂直方向能覆盖完整一圈，提供更高密度的三维数据。
#define COMPASS_CAL_SPACING_SAMPLES         100     // 仅用于 accept_sample() 间距公式

// 两阶段姿态约束采样：阶段0=水平旋转，阶段1=机头朝下旋转
// 水平采集50个点，机头朝下采集50个点（总共100点）
#define COMPASS_CAL_PHASE1_SAMPLES      50
