...
void CompassCalibrator::pull_sample()
{
    ...
    // 阶段0：样本足够且 IMU 检测到已旋转 ≥360° 才切换到阶段1
    if (_phase == 0) {
        _phase1_samples++;
        if (_phase1_samples >= COMPASS_CAL_PHASE1_SAMPLES &&
            _phase0_rot_accum >= (2.0f * M_PI)) {
            _phase = 1;
            _rot_last_time_ms = 0;  // 重置积分时间戳，避免跨阶段 dt 突变
...
bool CompassCalibrator::_fitting() const
{
    ...
    // 样本集满后还需等待阶段1（机头朝下）旋转满一圈才开始椭球拟合
    // 阶段0的旋转满圈已在 pull_sample() 里作为进入阶段1的前提条件
    return _phase1_rot_accum >= (2.0f * M_PI);
}
...
void CompassCalibrator::reset_state()
{
    ...
    _phase = 0;
    _phase1_samples    = 0;
    _phase0_rot_accum  = 0.0f;
    _phase1_rot_accum  = 0.0f;
    _rot_last_time_ms  = 0;
