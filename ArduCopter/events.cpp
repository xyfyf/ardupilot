...
// failsafe_odid_check - check for ODID failsafe
void Copter::failsafe_odid_check()
{
    ...
    if (odid == nullptr || !odid->enabled()) {
        failsafe.odid = false;
        return;
    }

    // Check if error contains 6 or 7
    bool has_6_or_7 = odid->has_error_code("67");

    if (has_6_or_7 && !failsafe.odid) {
        // Trigger RTL
        failsafe.odid = true;
        ...
        gcs().send_text(MAV_SEVERITY_WARNING, "ODID: Failsafe RTL triggered");
    } else if (!has_6_or_7 && failsafe.odid) {
        // Clear failsafe and switch to LOITER
        failsafe.odid = false;
        ...
        gcs().send_text(MAV_SEVERITY_NOTICE, "ODID: Failsafe cleared, Loiter");
    }
}
#endif
