...
本 fork 专用，不属于上游 ArduPilot。这里的脚本**都只读日志**，不改任何东西。
...
```bash
python3 Tools/eft_log_analysis/log_fence_margin.py <log.bin> [--mode ALT_HOLD] [--csv out.csv]
```

围栏多边形**从日志的 `FNCE` 消息自取**，不用手工填。输出每个飞行模式的最小法向
余量、越界采样数、最大地速，以及逐次逼近事件（最近点余量、撞的是哪条边、进场峰值
速度、峰值处距边界、平均减速度）。

`~/UAV/tools/fence_margin.py` 是 SITL 专用的另一个脚本（只认 `runs/` 目录、多边形靠
外部传入、按 `LOIT_SPEED` 分段），两者不要混用。

三个口径上的坑，都已固化进脚本，不是可选项：

- **位置只用 `POS`，绝不回退到 `GPS`。** `GPS` 按实例分开记录（本机 RTK 主 + NMEA 副，
  实例 0/1），不按实例过滤就会把两台接收机交织成一条序列，算出根本不存在的越界。
  00000211 上曾据此误得「ALT_HOLD 越界 1.2 m」——线索是距离在 0.2 s 内在 +1.8 与
  −1.0 m 间跳变，而飞机只有 0.4 m/s。
- **距离是到边界的法向距离，不是到中心的径向距离。** 对多边形，径向距离既算错余量
  也判不出越界（正六边形边心距只有外接半径的 0.866 倍）。
- **按 `CTUN.ThO >= 0.15` 排除地面段**，与 `log_control_metrics.py` 同口径。不排除的话
  停机等待会被算进当时的模式；同一模式被打断成几段时按间隔求和而不是取首末跨度，
  否则会把中间别的模式的时长一并算进去。

实测（架次 00000211，2026-09-02）：

```
=== LOITER ===    在飞 129 s（分 2 段）  最小余量 1.99 m  越界 0  最大地速 5.03 m/s
=== ALT_HOLD ===  在飞  73 s             最小余量 1.61 m  越界 0  最大地速 8.29 m/s
```

取证报告见 `~/UAV/flight_quality/姿态层围栏复飞验证_20260902.md`。
